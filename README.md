...
| `securityContext.fsGroupChangePolicy`      | Set to `OnRootMismatch` to skip the recursive ownership change of mounted volumes when the root already has the expected group. Applied to handler pods and, with Galaxy releases that support the `k8s_fs_group_change_policy` runner parameter, to tool job pods. Unset by default. |
