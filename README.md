...
| `rabbitmq.enabled`                      | Set to `true` to use RabbitMQ for message passing between Galaxy processes in the deployment. `false` by default.                             |
| `rabbitmq.galaxy.user`                  | User within RabbitMQ for Galaxy. Defaults to `galaxy`.                                                                                        |
| `rabbitmq.galaxy.password`              | Password for the above user in RabbitMQ.                                                                                                      |
| `rabbitmq.galaxy.vhost`                 | Vhost in RabbitMQ to be used by the Galaxy dedicated user. Defaults to `galaxy`.                                                              |
