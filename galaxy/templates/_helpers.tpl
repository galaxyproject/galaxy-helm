...
{{- tpl "cd /cvmfs && curl -O https://raw.githubusercontent.com/CloudVE/cvmfs-main-zipped/master/current/zipped.tar.gz && tar -xzvf zipped.tar.gz && find /cvmfs/main.galaxyproject.org/shed_tools -type f -path "*/.hg/*" -exec cp -s --parents \{\} ./zipped \; && find /cvmfs/main.galaxyproject.org/shed_tools -type f -path "*/test-data/*" -not -path "*/.hg/*" -exec cp -s --parents \{\} ./zipped \; && find /cvmfs/main.galaxyproject.org/shed_tools -type f -path "*/images/*" -not -path "*/test-data/*" -not -path "*/.hg/*" -exec cp -s --parents \{\} ./zipped \; && install -o 101 -g 101 /galaxy/server/config/integrated_tool_panel.xml /galaxy/server/config/mutable/integrated_tool_panel.xml; install -o 101 -g 101 /galaxy/server/config/sanitize_whitelist.txt /galaxy/server/config/mutable/sanitize_whitelist.txt; if [ ! -f \"{{ .Values.persistence.mountPath }}/config/editable_shed_tool_conf.xml\" ]; then install -D -o 101 -g 101 /galaxy/server/config/shed_tool_conf.xml.sample {{ .Values.persistence.mountPath }}/config/editable_shed_tool_conf.xml; fi" $}}
{{- end -}}
